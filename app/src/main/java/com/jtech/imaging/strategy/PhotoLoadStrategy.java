...
public class PhotoLoadStrategy {
    ...
    public static int getStrategyWidth(Context context, int outWidth) {
        ...
        //判断缓存策略
        if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_AUTO) {//无视网络，根据屏幕适配
            return outWidth;
        } else if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_NET_AUTO) {//根据网络进行适配
    ...
    private static int getFixedWidth(int strategy, int outWidth) {
        if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_FULL) {//全尺寸
            return outWidth;
        } else if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_1080) {//最高1080
            return 1080 > outWidth ? outWidth : 1080;
        } else if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_720) {//最高720
            return 720 > outWidth ? outWidth : 720;
        } else if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_480) {//最高480
            return 480 > outWidth ? outWidth : 480;
        } else if (strategy == PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_200) {//最高200
    ...
    private static int getNetAutoWidth(Context context, int outWidth) {
        ...
        if (netType == ConnectivityManager.TYPE_WIFI) {//wifi状态下
            ...
            if (subType == NetUtils.NETWORK_CLASS_2_G) {//2G网络加载缩略图
                return getFixedWidth(PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_200, outWidth);
            } else if (subType == NetUtils.NETWORK_CLASS_3_G) {//3G网络加载480P图片
                return getFixedWidth(PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_480, outWidth);
            } else if (subType == NetUtils.NETWORK_CLASS_4_G) {//4G网络加载720P图片
                return getFixedWidth(PhotoLoad.PHOTO_LOAD_STRATEGY_FIXED_720, outWidth);
            }
        }
